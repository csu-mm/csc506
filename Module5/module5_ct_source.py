...
# In my implementation, I need to use a data structure with defined data types.
#  example: contentByID_dict: Dict[int, str] = {}
#             is a dictionary(hash table) having 'int' type for 'key' and 'str' type for 'value'.
# So, in my implementation, I'll effectively be using a dictionary wherever a hash table is needed.
#
...
class ContentMining:
    ...
    def AddContent(self, id:int, tag:str, content:str):
        if self.contentByID_dict.get(id) is None and len(content.strip()) > 0:
            self.contentByID_dict[id] = content.strip()
        
        if self.contentIDsByTag_dict.get(tag.strip().upper()) is None:
            self.contentIDsByTag_dict[tag.strip().upper()] = [id]
        else:
            self.contentIDsByTag_dict[tag.strip().upper()].append(id)
        
        if self.contentAttributesByID_dict.get(id) is None:
            self.contentAttributesByID_dict[id] = [len(content.strip()), 0]
        return
    
    def AddContentLike(self, id:int, like:int):
        if self.contentAttributesByID_dict.get(id) is not None:
            self.contentAttributesByID_dict[id][1] = self.contentAttributesByID_dict[id][1]+like        
        return
    ...
    def GetContentsByTag(self, tag:str) -> List[int]:
        result: int = []
        if self.contentIDsByTag_dict.get(tag.strip().upper()) is not None:            
            return self.contentIDsByTag_dict[tag.strip().upper()]
...
class ContentUser:
    ...
    def AddNewUser_User_Content_Interaction_Cache(self, uid: int, tag: str):
        if self.userInterests_dict.get(uid) is None:
            self.userInterests_dict[uid] = [tag.strip().upper()]
        else:
            if tag.strip().upper() in self.userInterests_dict[uid] == False:
                self.userInterests_dict[uid].append(tag.strip().upper())
        return
    
    def Update_User_Content_Interaction_Cache(self, uid: int, cid: int):
        if self.userInterests_dict.get(id) is not None:
            self.interactionLog_dict[(uid, cid)] = datetime.now()
        return    
...
if __name__ == "__main__":
    ...
    while userInput != "Q":
        ...
        for cid in contentIds:            
            cntnsText += ("\n\n" + str(docNum) + " ==> ") + contentMining.contentByID_dict[cid] + ("\nContent Likes: ")
            while likeCnt < contentMining.contentAttributesByID_dict[cid][1]:
                cntnsText += "*"
                likeCnt += 1
            likeCnt = -1
            docNum += 1        
